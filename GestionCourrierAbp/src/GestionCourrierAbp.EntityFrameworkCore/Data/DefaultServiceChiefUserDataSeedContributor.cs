...
public class DefaultServiceChiefUserDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    private const int ServiceChiefServiceId = 5;
    private const string ServiceChiefLogin = "chefservice";
    private const string ServiceChiefName = "Chef de service";
    private const string ServiceChiefEmail = "chefservice@gestioncourrier.local";
    ...
    public DefaultServiceChiefUserDataSeedContributor(
    ...
    {
        await EnsureServiceChiefServiceAsync();
        await EnsureBusinessUserAsync();
        await EnsureIdentityUserAsync();
    }

    private async Task EnsureServiceChiefServiceAsync()
    {
        var existing = await _serviceRepository.FindAsync(ServiceChiefServiceId);
        if (existing != null)
        {
            existing.NomService = ServiceChiefName;
            existing.Description = "Consultation globale en lecture seule";
            existing.Etage = "2eme";
...
VALUES
    ({ServiceChiefServiceId}, {ServiceChiefName}, {"Consultation globale en lecture seule"}, {"2eme"}, {"{}"}, {Guid.NewGuid().ToString("N")}, {DateTime.Now});
...
SET IDENTITY_INSERT [AppServices] OFF;");
    ...
    private async Task EnsureBusinessUserAsync()
    {
        ...
        var existing = await query.FirstOrDefaultAsync(x => x.Login == ServiceChiefLogin);
        if (existing != null)
        {
            existing.NomComplet = ServiceChiefName;
            existing.ServiceId = ServiceChiefServiceId;
        ...
        {
            NomComplet = ServiceChiefName,
            Login = ServiceChiefLogin,
            PasswordHash = HashPassword(DefaultPassword),
            ServiceId = ServiceChiefServiceId
        }, autoSave: true);
    }

    private async Task EnsureIdentityUserAsync()
    {
        var identityUser = await _identityUserManager.FindByNameAsync(ServiceChiefLogin);
