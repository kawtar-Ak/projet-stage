...
function mapLegacyUrlToAbp(url = '') {
  ...
  return url
    ...
    .replace(/^\/api\/acteursjudiciaires\/(export\/excel|export\/registre-modele|export\/archives|template-excel|import\/excel|import-archive\/preview|import-archive\/execute|upload-pdf|upload-document)(\?.*)?$/, '/api/acteursjudiciaires/$1$2')
    .replace(/^\/api\/acteursjudiciaires\/(\d+)\/retraits\/export\/excel$/, '/api/acteursjudiciaires/$1/retraits/export/excel')
    .replace(/^\/api\/acteursjudiciaires\/retraits\/(\d+)$/, '/api/acteursjudiciaires/retraits/$1')
